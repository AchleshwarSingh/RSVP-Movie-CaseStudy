...
SELECT 
		...
		SUM(CASE WHEN worldwide_gross_income IS NULL THEN 1 ELSE 0 END) AS worlwide_gross_income_nulls,
...
ON n.id = d.name_id INNER JOIN top_3_genres_movie_director 
...
ON rm.movie_id = m.id INNER JOIN ratings r 
ON m.id = r.movie_id 
...
ON r.movie_id = rm.movie_id INNER JOIN genre g 
ON g.movie_id = r.movie_id 
